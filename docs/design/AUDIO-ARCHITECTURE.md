...
| `src/hooks/useYouTubePlayer.ts` | web-io | The `YouTubePlayerHandle` interface itself, which is the port, plus two behavioral rules: the player must be created inside a live user gesture, and the pending load/volume queue replays work issued before `onReady`. | Everything else, and there is no native YouTube player, so a native shell carries the same IFrame API inside a WebView with a self-hosted proxy page and `allowsInlineMediaPlayback` at the config layer. |
...
| D2 | The capture-profile decision (NC choice, channels, sample rate, preset, DTX) is written out **nine times** across `connection.ts`, `capture.ts` and `micCheck.ts`. | new `src/lib/audioProfile.ts` | `resolveCaptureProfile({ micMode, talkingNC, singingNC, purpose })` returning the platform-free shape, plus a one-line `toMediaTrackConstraints(profile, deviceId)` web adapter. Nine literals become one tested function. | Half a day |
| D3 | `mixMicStream` is dead: `RoomView` never destructures it, yet six setter call sites keep it fed. | `context.ts`, `capture.ts`, `connection.ts`, `useLiveKit.ts` | Delete the field and the setter. Removes a `MediaStream` from the future port surface for free. | 15 minutes |
...
| D8 | Mic error copy and its 3s auto-clear are duplicated between `applyMicState`'s catch and `startSinging`'s catch. | new `src/lib/micErrors.ts` | `classifyMicError(err): { message, autoClearMs }`. Two call sites, one table. Touches zero graph wiring. | 1 hour |
...
- **A rule written twice is a rule that will diverge.** The capture profile is written nine times today (debt D2) and it is the concrete reason this section exists: the day singing NC changes shape, eight of the nine will be updated and one will not, and the symptom will be one code path publishing mono.
