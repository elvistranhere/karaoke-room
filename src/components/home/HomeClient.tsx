...
const HIGHLIGHTS = [
  {
    title: "Everyone hears the same beat",
    body: "Whoever is on stage drives playback and every other browser follows, so the YouTube backing track stays lined up across the room.",
  },
  {
    title: "Live voice with effects",
    body: "Your microphone goes out to the room in real time, with hall, echo, warmth and chorus you can dial in while you sing.",
  },
  {
    title: "Private unless you say so",
    body: "A room is reachable only by its code, you can lock it with a password, and listing it in Browse is a choice you make.",
  },
];
...
export function HomeClient() {
  ...
  return (
    <main className="relative flex flex-col items-center overflow-hidden">
      ...
      <div className="relative flex min-h-[calc(100dvh-3rem)] w-full flex-col items-center justify-center pb-[max(env(safe-area-inset-bottom),2.5rem)] pl-[max(env(safe-area-inset-left),1rem)] pr-[max(env(safe-area-inset-right),1rem)] pt-[max(env(safe-area-inset-top),2.5rem)]">
      ...
      <section
        aria-labelledby="about-karaoke-now"
        className="w-full pb-[max(env(safe-area-inset-bottom),3.5rem)] pl-[max(env(safe-area-inset-left),1rem)] pr-[max(env(safe-area-inset-right),1rem)]"
      >
        <div className="mx-auto w-full max-w-2xl">
        <h2
          id="about-karaoke-now"
          className="text-xl font-bold tracking-tight sm:text-2xl"
          style={{ fontFamily: "var(--font-display)", color: "var(--color-text-primary)" }}
        >
          Free online karaoke rooms for singing with friends
        </h2>
        <p className="mt-3 text-sm leading-6" style={{ color: "var(--color-text-secondary)" }}>
          Karaoke Now turns any browser into a shared karaoke room. Create a room, send the
          six-character code to whoever you want singing, and everyone lands on the same stage
          within seconds. There is no app to install, no account to make and nothing to pay,
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {HIGHLIGHTS.map((item) => (
            <div
              key={item.title}
              className="rounded-xl p-4 shadow-[var(--shadow-elevation-0)]"
        ...
        <p className="mt-5 text-sm leading-6" style={{ color: "var(--color-text-secondary)" }}>
          It works for long-distance friends, family across time zones, game nights and office
          parties: anywhere people want to sing together without being in the same room. Want to
          jump straight in? <Link href="/browse" className="underline underline-offset-2" style={{ color: "var(--color-primary-soft)" }}>Browse public karaoke rooms</Link> and join one that is already live.
        </p>
        </div>
      </section>
    </main>
