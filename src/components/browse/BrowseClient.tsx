  ...
  return (
    <main className="relative flex min-h-dvh flex-col items-center overflow-hidden pb-[max(env(safe-area-inset-bottom),2.5rem)] pl-[max(env(safe-area-inset-left),1rem)] pr-[max(env(safe-area-inset-right),1rem)] pt-[max(env(safe-area-inset-top),2.5rem)]">
      ...
      <div className="mb-8 w-full max-w-2xl" style={{ animation: "fade-in 0.5s ease-out" }}>
        <Link
          href="/"
          className="mb-2 flex min-h-10 w-fit items-center gap-1.5 py-2 text-sm transition-colors hover:brightness-125"
        ...
        <p className="mt-1.5 text-xs leading-5" style={{ color: "var(--color-text-muted)" }}>
          Karaoke rooms that are open to anyone right now. Pick one, take the mic, and sing
          along with whoever is already there. No signup needed.
        </p>
      </div>
